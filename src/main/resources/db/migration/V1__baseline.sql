...
-- Postgres 15+. Based on passlify-spec/DOMAIN.md §2/§3, adapted for the build
-- decisions taken at kickoff:
--   * MULTI-CURRENCY: per-event `currency`, no hardcoded EUR check. Default RSD
--     (Serbia-first per the master spec). One currency per order, enforced in
--     the service layer.
--   * MONEY = BIGINT minor units (para/cents). BIGINT not INT, because RSD
--     nominal values are ~117x EUR and INT would cap at ~21M minor units.
--   * LIGHTWEIGHT TENANCY: `organizer_id` is the Keycloak `sub` (no org tables
--     yet). Full organizations/members slot in later as a migration.
--   * PLUGGABLE PAYMENTS: `event.payment_provider` selects the adapter; the
--     provider enum starts small (MOCK/MANUAL/STRIPE) and grows by migration.
-- Columns are snake_case (Spring Boot default physical naming maps camelCase
-- entity fields here). Enums are varchar + CHECK to line up with
...
CREATE TABLE event (
    id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug             VARCHAR(120),                          -- generated lowercase, globally unique
    ...
    CONSTRAINT ck_event_provider      CHECK (payment_provider IN ('MOCK','MANUAL','STRIPE')),
...
CREATE TABLE ticket_type (
    ...
    visibility     VARCHAR(20)  NOT NULL DEFAULT 'PUBLIC',  -- public vs hidden (master spec §6.6)
    ...
    CONSTRAINT ck_tt_visibility    CHECK (visibility IN ('PUBLIC','HIDDEN')),
...
CREATE TABLE orders (
    ...
    expires_at          TIMESTAMPTZ,               -- reservation TTL (checkout hold)
    paid_at             TIMESTAMPTZ,
    meta                JSONB,
    created_at          TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ  NOT NULL DEFAULT now(),
...
CREATE TABLE order_item (
    ...
    unit_price_minor  BIGINT NOT NULL,   -- snapshot at purchase time
    total_price_minor BIGINT NOT NULL,   -- unit_price_minor * quantity
    meta              JSONB,
    created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE payment (
    ...
    created_at               TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at               TIMESTAMPTZ  NOT NULL DEFAULT now(),
    CONSTRAINT ck_payment_provider CHECK (provider IN ('MOCK','MANUAL','STRIPE')),
...
CREATE INDEX idx_payment_session ON payment (provider_session_id);
CREATE INDEX idx_payment_intent  ON payment (provider_intent_id);
CREATE INDEX idx_payment_charge  ON payment (provider_charge_id);

-- The dedup guard: every received provider webhook event is recorded before
-- processing. Provider-agnostic (was Stripe-only in the spec).
CREATE TABLE webhook_event (
    id           VARCHAR(255) NOT NULL,    -- provider event id (evt_..., or adapter-generated)
    provider     VARCHAR(20)  NOT NULL DEFAULT 'MOCK',
    type         VARCHAR(120) NOT NULL,
    payload      JSONB,
    received_at  TIMESTAMPTZ  NOT NULL DEFAULT now(),
    processed_at TIMESTAMPTZ,
    CONSTRAINT pk_webhook_event PRIMARY KEY (provider, id)
);
