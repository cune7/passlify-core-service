...
- **Raiffeisen (UPC e-Commerce Connect Gateway) — CORE DONE, config-gated.** Raiffeisen RS actually uses the UPC gateway (RSA-SHA1 signed redirect), NOT Payten/NestPay — the earlier NestPay attempt was wrong and was replaced. `UpcSignature` (RSA-SHA1) + `RaiffeisenPaymentGateway` (signed request fields + NOTIFY signature verify + TranCode→PAID/FAILED) implemented per the merchant doc (`passlify-spec/docs/rfz-ecommerce-integration-docs`) and unit-tested with generated keypairs. Inert until `passlify.raiffeisen.enabled=true` + keys. Full flow now built: `RaiffeisenRedirectController` (auto-submit POST form) + `RaiffeisenNotifyController` (signature-verified `Response.action=approve`/`reverse` handshake), tested via enabled-path MockMvc. Webhook payload column is now text (V13) so form-encoded bodies persist. Remaining = live verification against the UPC test server with real creds (field order, exact NOTIFY body, form-encoding of the base64 Signature).
- **Stripe (stripe-java) — BUILT, config-gated.** `StripePaymentGateway` (Checkout Session create + `Webhook.constructEvent` signature verify, mapping `checkout.session.completed`/`payment_intent.succeeded`→PAID, `.payment_failed`→FAILED, `charge.refunded`→REFUNDED) + `StripeWebhookController` (`POST /api/v1/webhooks/stripe`, reads `Stripe-Signature`). Inert until `passlify.stripe.enabled=true` + keys. Unit + enabled-path MockMvc tests. Needs Stripe test-mode keys for live end-to-end verification.

Full payments runbook (providers, capabilities, gateway model, Raiffeisen/UPC config + flow + open items): `passlify-spec/PAYMENTS.md`.
