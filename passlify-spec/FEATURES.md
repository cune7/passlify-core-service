...
- 🟡 **Raiffeisen gateway (Payten/NestPay 3D hosted page):** `RaiffeisenPaymentGateway` + `NestPayHash` (ver3) implemented and unit-tested, but **config-gated** (`passlify.raiffeisen.enabled`, inert until merchant creds set) and **unverified against the bank** — field set / hash version / result codes need confirming against the merchant integration doc + a test-env store key.
